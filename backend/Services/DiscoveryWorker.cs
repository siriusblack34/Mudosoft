...
    {
        // ❌ IDeviceRepository kaldırıldı.
        private readonly IServiceScopeFactory _scopeFactory; // 🔥 YENİ: Factory eklendi.
...
        // Constructor sadece Logger ve Factory alır.
...
        {
            // 🔥 YENİ: Burası değişti! Scope oluşturulur ve Repository bu scope'tan alınır.
...
                {
                    try { await ScanStore(storeCode, token, repo); } // repo parametre olarak gönderilir
                    catch (Exception ex)
                    {
...
        }
        ...
        // 🔥 YENİ: repo parametresi ScanStore metoduna eklendi.
        private async Task ScanStore(int storeCode, CancellationToken ct, IDeviceRepository repo) 
        {
...
                try
                {
                    // repo artık parametreden geliyor.
...
                    repo.SaveAll(devices); // repo artık parametreden geliyor.
                }
                catch (Exception ex)
                {
